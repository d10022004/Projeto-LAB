...
def tab(jogadores):
    ...
    # Define the cell coordinates in the dicionario dictionary
    lar = 55
    com = 60
    for h in range(1, 31):
        if h <= 10:
    ...
    def move_button(button):
        global resultado
        nonlocal pecas_out_branco, pecas_out_preto
        ...
        if button_clickable[button]:
            ...
            for piece in white_pieces:
                if piece.player == jogador1 and button_positions[button] == white_pieces.index(piece) + 1:
                    white_pieces.remove(piece)
                    white_pieces.insert(new_position - 1, piece)
                    break

            for piece in black_pieces:
                if piece.player == jogador2 and button_positions[button] == black_pieces.index(piece) + 1:
                    black_pieces.remove(piece)
                    black_pieces.insert(new_position - 1, piece)
                    break
